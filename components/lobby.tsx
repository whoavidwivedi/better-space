          ...
          <div className="flex w-full shrink-0 flex-wrap items-center justify-between gap-2.5 sm:flex-nowrap sm:justify-end sm:gap-3 md:w-auto">
            ...
            <CharacterPicker
              ...
              trigger={
                <Button
                  variant="outline"
                  className="h-10 rounded-xl pr-3.5 pl-2 shadow-xs font-mono text-xs font-bold tracking-wider text-foreground uppercase"
            ...
            <Popover open={isCreateOpen} onOpenChange={setIsCreateOpen}>
              <PopoverTrigger
                render={
                  <Button className="h-10 gap-2 rounded-xl bg-foreground px-4 font-mono text-xs font-bold tracking-wider text-background uppercase hover:bg-foreground/90 shadow-sm">
              ...
              >
                <PopoverHeader className="mb-4">
                  <div className="flex items-center gap-2">
                    <RiTeamLine size={16} className="text-foreground shrink-0" />
                  ...
                  <PopoverDescription className="mt-1 font-mono text-[10px] text-muted-foreground leading-normal">
                ...
                >
                  <Field className="space-y-1.5">
                    <div className="flex items-baseline justify-between">
                      <FieldLabel
                        ...
                        className="font-mono text-[10px] font-bold text-muted-foreground uppercase tracking-wider"
                      >
                        Space Name
                      </FieldLabel>
                      <span
                  ...
                  <div className="flex items-center justify-between gap-4 pt-4 mt-1.5 border-t border-border/40">
                    {createUserName.trim() &&
                    ...
                    <Button
                      type="submit"
                      className="h-10 shrink-0 rounded-xl bg-foreground px-5 font-mono text-xs font-bold tracking-wider text-background uppercase hover:bg-foreground/90 shadow-sm focus-visible:ring-2 focus-visible:ring-foreground/25"
                      disabled={
                        !newSpaceName.trim() ||
                        !createUserName.trim() ||
                        isJoining ||
                ...
                >
                  ...
                  <div className={`w-full ${timeLeft > 0 ? "mt-4" : ""}`}>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-1.5 rounded-md border border-foreground/15 bg-muted/65 px-2 py-0.5 font-mono text-[9px] font-bold text-foreground uppercase tracking-wider">
                        <span className="relative flex size-1.5">
                          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-foreground/25 opacity-75"></span>
                      ...
                      <div className="flex items-end gap-[2px] h-3.5 w-4 shrink-0">
                        <span className="w-[2px] bg-foreground/80 rounded-full animate-eq-1" style={{ height: "30%" }} />
                        <span className="w-[2px] bg-foreground/80 rounded-full animate-eq-2" style={{ height: "70%" }} />
                        <span className="w-[2px] bg-foreground/80 rounded-full animate-eq-3" style={{ height: "50%" }} />
                        <span className="w-[2px] bg-foreground/80 rounded-full animate-eq-4" style={{ height: "90%" }} />
                      </div>
                    </div>

                    {/* Room title & Host badge */}
                    <div className="space-y-3">
                      <h3 className="font-display text-lg font-extrabold tracking-tight text-foreground line-clamp-1 group-hover:text-primary transition-colors sm:text-xl">
                        {getDisplayRoomTitle(room.name)}
                      </h3>
                      
                      {/* Host identity tag */}
                      <div className="inline-flex items-center gap-2 rounded-lg border border-border bg-muted/40 pr-2.5 pl-1.5 py-0.5 text-xs">
                        ...
                        <span className="font-mono text-[9px] font-semibold text-muted-foreground">
                          host: <span className="text-foreground font-bold">{room.host}</span>
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex w-full items-center justify-between gap-4 mt-auto">
                    {/* Avatars */}
                    <div className="flex shrink-0 -space-x-1.5">
                      {room.participants.slice(0, 4).map((p, idx) => (
                        <div
                          ...
                          style={{ zIndex: 4 - idx, transitionDelay: `${idx * 50}ms` }}
                        >
                          <img
                      ...
                      {room.participants.length > 4 && (
                        <div className="flex size-7.5 items-center justify-center rounded-full border-2 border-card bg-muted font-mono text-[9px] font-bold text-foreground shadow-xs" style={{ zIndex: 0 }}>
                          +{room.participants.length - 4}
                        </div>
                      )}
                    </div>
                    ...
                    {/* CTA Join Button */}
                    <div className="flex items-center gap-1.5 rounded-xl bg-foreground px-3 py-1.5 font-mono text-[10px] font-bold tracking-wider text-background uppercase shadow-sm group-hover:bg-foreground/90 transition-all">
                      <span>{room.numParticipants} {room.numParticipants === 1 ? "speaker" : "speakers"}</span>
          ...
          >
            <Field className="space-y-1.5">
              <div className="mt-1 mb-2 flex items-center justify-center gap-3">
                ...
                <Button
                  ...
                  className="h-9 rounded-xl font-mono text-xs font-bold tracking-wider uppercase text-foreground"
            ...
            <div className="pt-2">
              ...
              >
                {isJoining ? <Spinner className="mr-2" /> : null}
                <span>
                  {rooms.find((r) => r.name === selectedRoom)?.host === joinUserName.trim()
                    ? "Go Live"
                    : "Join Space"}
                </span>
              </Button>
