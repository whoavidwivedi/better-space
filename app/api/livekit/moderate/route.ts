...
export async function POST(req: NextRequest) {
  ...
  const cleanRoom = roomName.trim().substring(0, 50)
  ...
  switch (action) {
    ...
    case "end":
      {
        const known = getKnownRoom(cleanRoom)
        let participantsList: string[] = known?.participants || []
        try {
          const liveParticipants = await roomService.listParticipants(cleanRoom)
          if (liveParticipants.length > 0) {
            participantsList = liveParticipants.map((p) => p.identity)
          }
        } catch {}

        addRecentSpace({
          name: cleanRoom,
          displayName: meta?.title || known?.displayName || cleanRoom,
          host: meta?.host || identity,
          numParticipants:
            targetRoom?.numParticipants || participantsList.length || 1,
          participants: participantsList,
          endedAt: Date.now(),
          endedByHost: true,
        })
        deleteKnownRoom(cleanRoom)
    ...
    case "grant_mic":
      ...
      try {
        const liveParticipants = await roomService.listParticipants(cleanRoom)
        const activeSpeakers = liveParticipants.filter(
          (p) => p.permission?.canPublish
        )
        if (
          activeSpeakers.length >= 8 &&
          !activeSpeakers.some((p) => p.identity === targetIdentity)
        ) {
          return NextResponse.json(
            {
              error: {
                message:
                  "Stage is full (max 8 speakers allowed including host and co-hosts).",
              },
            },
            { status: 400 }
          )
    ...
    case "mute":
      ...
      try {
        ...
        for (const track of participant.tracks) {
          if (track.source === 2 || track.type === 0) {
