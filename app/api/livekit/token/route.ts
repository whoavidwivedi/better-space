...
export async function GET(req: NextRequest) {
  ...
  const cleanRoom = room.trim().substring(0, 60)
  ...
  if (targetRoom && targetRoom.metadata) {
    try {
      ...
      if (meta.hostSecret && hostSecret === meta.hostSecret) {
        if (meta.host && username !== meta.host) {
          return NextResponse.json(
            {
              error: {
                message: `Cannot change host nickname when rejoining. Your host name is "${meta.host}".`,
              },
            },
            { status: 403 }
          )
        ...
        isHost = true
      } else if (meta.host && meta.host === username) {
        // Someone trying to claim host username without secret
        return NextResponse.json(
          {
            error: {
              message: "This username is reserved for the host of this space.",
            },
          },
          { status: 403 }
        )
  ...
  const avatarQuery = req.nextUrl.searchParams.get("avatar") || "Felix"
  const avatar = avatarQuery.trim().substring(0, 50) || "Felix"
